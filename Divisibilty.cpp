...
*/
ll dp[100001][101];
bool getAns(vector<ll>& arr,ll curr,ll n,ll sum,ll k){
    ...
    bool a = getAns(arr,curr+1,n,(sum+arr[curr])%k,k);
    
    if(curr!=0){
        a = a || getAns(arr,curr+1,n,(sum-arr[curr]+k)%k,k);
    }

    return dp[curr][sum] = a;
}
