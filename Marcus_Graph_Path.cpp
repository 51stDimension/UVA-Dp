...
No matter how many times you have to submit. 
...
*/
const int xMoves[] = {-1, 0, 0};
const int yMoves[] = {0, -1, 1};
const string moveNames[] = {"forth", "left", "right"};

bool getAns(vector<vector<char>>& arr,vector<string>& op,string s,ll start,ll x,ll y,ll r,ll c){
    if(arr[x][y]=='#'){
        cout<<op[0];
    ...
    for(ll i=0;i<3;i++){
        ...
        if(nextx>=0 && nexty>=0 && nextx<r && nexty<c && arr[nextx][nexty]==s[start]){

            op.pb(moveNames[i]);
            if(getAns(arr,op,s,start+1,nextx,nexty,r,c)){
                return true;
            }
            op.pop_back();
        }
...
void solve(ll tc){
    ...
    bool a = getAns(arr,ans,s,0,x,y,r,c);
}
