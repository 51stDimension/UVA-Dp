...
*/
vector<ll> nodes;
bool dfs(map<ll,vector<ll>>& adj,ll s,map<ll,bool>& visited,map<ll,bool>& recst){
    visited[s] = true;
    recst[s] = true;
    for(auto u:adj[s]){
        if(!visited[u] && dfs(adj,u,visited,recst)){
            return true;
        }
        else if(recst[u]){
            return true;
...
bool dfsCycleHelper(map<ll,vector<ll>>& adj){
    map<ll,bool> visited,recst;

    for(auto e:nodes){
        if(adj[e].size()!=0 && !visited[e]){
            if(dfs(adj,e,visited,recst)){
                return true;
            }
        }
    }
...
void solve(ll tc){
    ...
    while(true){
        ...
        if(dfsCycleHelper(adj) || root_count>1){
            istree = false;
        }
        if(istree){
